...
export default function Changepassword() {
    ...
    const changePassword = async (data: UserChangePassword) => {
        ...
        setLoading(true);
        try {
            const request = axios.post(`${process.env.NEXT_PUBLIC_API_URL}/user/changepassworduser?`, {
    ...
    return (
        <div className='w-full h-fit flex flex-col gap-4 bg-cream shadow-sm p-6 text-amber-950'>
            <h2 className='text-lg font-semibold'>Change password</h2>

            <form onSubmit={handleSubmit(changePassword)} className='flex flex-col gap-1 text-amber-950'>
                ...
                <label htmlFor="" className='text-sm mt-4'>Confirm password</label>
                <div className="relative">
                    <Input
                ...
                <Button disabled={loading} className=' w-full font-medium mt-4 flex items-center justify-center gap-2'>
                    {loading === true && (
