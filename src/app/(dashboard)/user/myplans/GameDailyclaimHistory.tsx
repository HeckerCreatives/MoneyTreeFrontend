...
export default function DailyClaimHistory() {
    ...
    useEffect(() => {
        ...
        const getList = async () => {
          try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/inventory/dailyclaimhistory?page=${currentpage}&limit=10`,{
  ...
  return (
     <div className=' w-full flex flex-col gap-4 h-auto bg-cream rounded-xl shadow-sm p-6 mt-12'>
        <p className=' text-sm font-medium'>Daily Claim History</p>
            <Table>
                {loading === true && (
                    <TableCaption>
