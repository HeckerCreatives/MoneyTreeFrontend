...
export default function Gamewithdrawhistory() {
    ...
    useEffect(() => {
        ...
        const getList = async () => {
          try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/payout/getrequesthistoryplayer?type=gamebalance&page=${currentpage}&limit=10`,{
  ...
  return (
     <div className=' w-full flex flex-col gap-4 h-[500px]  rounded-xl shadow-sm mt-4'>
        <p className=' text-sm font-medium'>Game Withdraw History</p>
            <Table>
                {loading === true && (
                    <TableCaption>
